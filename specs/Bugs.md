...
| BUG-S-019 | 2026-09-23 | Flyout (sidebar.xml:72 + sidebar.js:192) | **Flyout se cierra al mover el mouse dentro de él.** `t-on-mouseleave` del botón rail dispara `clearHover()` (timer 180ms); el flyout no tiene `mouseenter` que cancele el timer → al pasar del rail al flyout, este se cierra a los ~180ms (imposible clicar submenú con mouse). Bug de UX real. Fuente: `_clearHoverTimer` solo se limpia en `hoverApp()` (fired on rail button enter), no en flyout enter. **Reporte cliente 2026-09-23:** "es complejo ingresar a un apartado desde la barra lateral... el submenú se va demasiado rápido y no permite seleccionar". Fix propuesto pendiente: `t-on-mouseenter` en el flyout cancelando `_clearHoverTimer` (o `hoverApp(state.flyoutApp)`). | UI/UX | 🟡 Media | Abierto |
| BUG-S-021 | 2026-09-23 | Breakpoints (sidebar.scss:92, sidebar.xml:7) | **Gap 769-991px sin acceso a apps.** Sidebar visible solo ≥992px (`d-lg-flex`), toggle drawer solo ≤768px (`@media max-width:768`). Entre 769 y 991px: sidebar oculto Y toggle oculto → usuario sin navegación de apps. Afecta tablets portrait, ventanas medianas y **viewport default de Puppeteer (800×600)**. | UI/UX | 🟡 Media | Abierto |
...
| BUG-S-020 | 2026-09-23 | Test infra (cdp_layout.js / cdp_smoke.js) | **Falso negativo de layout por viewport por defecto.** `cdp_layout.js` y `cdp_smoke.js` no llaman `page.setViewport` → Puppeteer usa 800×600. Sidebar (`d-lg-flex`, oculto <992px) → `display:none`. Informe decía "Sidebar oculto a 1024px — d-lg-flex no aplica" (falso: era 800px). Con viewport real 1024: sidebar visible, 60px, R2 OK. `cdp_smoke.js` cuenta `.o_erpico_rail_btn` con `$$` (DOM, no visibilidad) → riesgo falso positivo en visibilidad. | QA/test | 🟡 Media | Abierto (herramienta) |
| BUG-S-023 | 2026-09-23 | Topbar (navbar.xml + spec §5.2) | **Mejora (feedback cliente): restaurar menú del módulo en la topbar.** La topbar v1 (patch template `web.NavBar`) eliminó AppsMenu, brand, breadcrumbs y secciones → logo ERPICO + systray nomás. Al entrar a un módulo falta orientación (breadcrumb, nombre del módulo, secciones). Fix: conservar en el template `navbar.xml` el brand del módulo (`currentApp.name`), el portal de breadcrumbs (`.o_navbar_breadcrumbs`) y `<t t-call="web.NavBar.SectionsMenu">` (gate `!this.ui.isSmall`); AppsMenu del core NO se restaura (el rail/drawer lo sustituye). Revierte parcialmente D-14 → **D-28**. Solo template, sin JS (`currentApp`/`currentAppSections` ya expuestos). | UI/UX | 🟡 Media | Abierto (mejora) |
