...
| BUG-S-023 | 2026-09-23 | Topbar (navbar.xml + spec §5.2) | **Mejora (feedback cliente): restaurar menú del módulo en la topbar.** La topbar v1 eliminó AppsMenu, brand, breadcrumbs y secciones → logo ERPICO + systray nomás. **Resuelto:** `navbar.xml` restaura brand del módulo (`currentApp.name`), breadcrumbs y SectionsMenu. AppsMenu NO se restaura (rail lo sustituye). Solo template, sin JS. | UI/UX | 🟡 Media | Resuelto |
